...
function generateAlerts(metrics: MetricInput): Alert[] {
  ...
  if (roas && roas < 2.0) {
    alerts.push({
      ...
      descripcion: `ROAS actual: ${roas.toFixed(2)}x. Con los costos de COD en tu país, el mínimo para no perder plata es ~2.2x.`,
  ...
  if (entrega && entrega < 65) {
    alerts.push({
      ...
      descripcion: `El promedio COD en ${cfg?.nombre || 'tu país'} es 70-80%. Por debajo de 65% estás perdiendo plata en fletes sin cobrar.`,
