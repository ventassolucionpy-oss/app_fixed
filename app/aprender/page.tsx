...
const SECCIONES = [
  ...
  {
    titulo: '👥 Equipo y operación a escala',
    desc: 'Cómo pasar de solo vos a un equipo que opera sin que estés en todo',
    color: 'border-indigo-500/20',
    items: [
      { href: '/equipo', icon: '👥', bg: 'bg-gradient-to-br from-violet-600 to-indigo-600', titulo: 'Roles del equipo y sus SOPs', desc: 'Cuándo contratar a cada rol, qué puede hacer cada uno, y los protocolos paso a paso para que operen sin consultarte todo.' },
      { href: '/rutina-diaria', icon: '⚡', bg: 'bg-gradient-to-br from-amber-600 to-orange-600', titulo: 'La rutina diaria del operador de 7 cifras', desc: 'El checklist de 15 minutos que hacen los que escalan: control de números, decisiones de campaña, producción y revisión de tarde.', badge: '🆕 Nuevo' },
      { href: '/pipeline-creativos', icon: '🎬', bg: 'bg-gradient-to-br from-rose-600 to-pink-600', titulo: 'Pipeline de producción de creativos', desc: 'A 7 cifras necesitás 8-12 creativos nuevos por semana. El sistema kanban para gestionarlo sin que nada se caiga.', badge: '🆕 Nuevo' },
    ],
  },
  {
    titulo: '💸 Finanzas reales del dropshipping',
    desc: 'Los números que la mayoría ignora hasta que se queda sin capital',
    color: 'border-emerald-500/20',
    items: [
      { href: '/flujo-caja', icon: '💸', bg: 'bg-gradient-to-br from-emerald-600 to-teal-700', titulo: 'Flujo de caja y capital de trabajo', desc: 'Por qué Dropi paga cada 15 días y Meta cobra diario es el problema real al escalar. Simulación exacta de cuánto capital necesitás.', badge: '🆕 Nuevo' },
      { href: '/dashboard-pl', icon: '💰', bg: 'bg-emerald-700', titulo: 'Leer el P&L real del día', desc: 'La diferencia entre el ROAS que muestra Meta y la ganancia real después de rechazos, fletes y costos ocultos.' },
    ],
  },
  {
    titulo: '🛡️ Protección de cuenta Meta',
    desc: 'Lo que nadie enseña hasta que te banean',
    color: 'border-red-500/20',
    items: [
      { href: '/proteccion-cuenta', icon: '🛡️', bg: 'bg-gradient-to-br from-red-700 to-red-800', titulo: 'Arquitectura anti-ban y calentamiento', desc: 'Cómo estructurar tu BM, cuántas cuentas tener, cómo calentar cuentas nuevas sin bans, y qué hacer si te banean.', badge: '🔴 Crítico' },
      { href: '/meta-rechazos', icon: '🚫', bg: 'bg-red-600', titulo: 'Qué evitar para no recibir rechazos', desc: 'Los 5 patrones de rechazo más comunes por nicho (salud, urgencia, financiero, ropa, general). Qué hace Meta y por qué.', badge: '🆕 Nuevo' },
    ],
  },
  {
    titulo: '📞 Operación COD — Confirmación y cierre',
    desc: 'El paso que más plata deja sobre la mesa si no lo hacés',
    color: 'border-teal-500/20',
    items: [
      { href: '/confirmacion-cod', icon: '📞', bg: 'bg-gradient-to-br from-teal-600 to-cyan-700', titulo: 'Scripts de llamada de confirmación COD', desc: 'Llamar antes de enviar a Dropi sube la tasa de entrega del 70% al 85%. Scripts completos para PY, CO y MX en 4 situaciones.', badge: '💰 +15% margen' },
    ],
  },
]
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-white/40 text-sm mt-1">Todo lo que necesitás aprender para vender en {cfg?.nombre || 'tu país'} {cfg?.bandera || '🌎'}</p>
        </div>

        {/* Stats banner */}
        <div className="grid grid-cols-3 gap-2 mb-6">
          <div className="card p-3 text-center rounded-xl border border-violet-500/20">
            <p className="text-lg font-bold text-violet-400">15</p>
