...
function buildPrompt(product: Product, config: Record<string, string>, modules: ModuleKey[]): string {
  ...
  const defs: Record<ModuleKey, string> = {
    ...
    "estructura_campana": "descripción de la estructura: CBO vs ABO, cuántos adsets, cuántos ads por adset, lógica de presupuesto para COD",
    "adsets": [
    ...
    "configuracion_pixel": "eventos de conversión específicos para Dropi COD",
    "tip_dropi": "tip específico para COD en este mercado que aumenta el ROAS con Dropi"
  }`,
...
BRIEF: Producto: ${product.name} | Descripción: ${product.description} | Público: ${publico} | Nivel de conciencia: ${config.nivel_conciencia || 'Nivel 3'} | Objetivo: ${config.objetivo || 'Generar ventas'} | Presupuesto: ${config.presupuesto || 'USD 10-30/día'} | País: ${pais || config.pais || 'PY'} | Moneda: ${cfg?.moneda || 'PYG'} | Contexto: ${cfg?.contexto_cultural?.slice(0,100) || ''}
              ...
              <div className="card p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    { key:'pais', label:'País/Región', opts:['Paraguay','Colombia','México','LATAM completo'] },
                  ].map(({key,label,opts})=>(
                    <div key={key}>
