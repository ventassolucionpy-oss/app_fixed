...
async function callClaude(data: Record<string, string>): Promise<CheckoutOutput> {
    const prompt = `Sos un experto en copywriting de checkout para dropshipping COD en ${cfg?.nombre || 'Paraguay'} con Dropi. Tu especialidad es escribir descripciones que conviertan visitantes en compradores en el momento final — cuando el cliente ya está en la página del producto y está a punto de llenar su nombre y teléfono.
...
Generá el contenido completo del checkout de Dropi. Respondé SOLO con JSON válido:
{
  ...
  "garantia": "texto de garantía específico para COD ${cfg?.nombre || ''} — honesto y tranquilizador",
  "preguntas_frecuentes": [
    { "pregunta": "¿Cómo funciona el pago?", "respuesta": "respuesta específica para COD ${cfg?.nombre || ''}" },
