...
async function callClaude(data: Record<string, string>): Promise<CreativoVisualOutput> {
    const prompt = `Sos un director de arte y media buyer experto en creativos para Meta Ads y TikTok Ads, especializado en dropshipping COD en ${cfg?.nombre || 'Paraguay'}. Sabés exactamente qué elementos visuales paran el scroll en el feed paraguayo.
...
Generá la estrategia visual completa. Respondé SOLO con JSON válido:
{
  ...
  "errores_visuales_comunes": [
    "error 1 que cometen los dropshippers en ${cfg?.nombre || 'Paraguay'} al hacer creativos",
    "error 2",
    "error 3",
    "error 4"
  ],
