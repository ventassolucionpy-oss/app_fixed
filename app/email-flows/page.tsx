...
PRODUCTO: ${product.name} | Descripción: ${product.description} | Precio: ${config.precio || 'no especificado'} | País: ${config.pais || pais || 'Paraguay'} | Público: ${config.publico || 'compradores online'}
          ...
          <div className="space-y-5 animate-fade-up">
            ...
            <div className="card p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-medium text-white/40 mb-1.5">País</label><select className="input cursor-pointer" value={config.pais || pais || 'Paraguay'} onChange={e => setConfig(p => ({ ...p, pais: e.target.value }))}>{['Paraguay','Argentina','Uruguay','Chile','Colombia','México','LATAM'].map(o => <option key={o} value={o} style={{ background: '#111' }}>{o}</option>)}</select></div>
              </div>
            </div>
