...
function autoPublico(p: Product, pais: string = 'PY'): string {
  ...
  if (d.match(/belleza|skin|crema|piel/)) return `Mujeres 25-45 interesadas en skincare, ${pais === 'CO' ? 'Colombia' : pais === 'MX' ? 'México' : 'Paraguay'}`
  if (d.match(/tecnología|gadget/)) return `Hombres 18-40 tech enthusiasts, ${pais === 'CO' ? 'Colombia' : pais === 'MX' ? 'México' : 'Paraguay'}`
  return `Compradores online 22-45, clase media, ${pais === 'CO' ? 'Colombia' : pais === 'MX' ? 'México' : 'Paraguay'}`
}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select className={`${inputClass} cursor-pointer`} value={config.pais || ''} onChange={e => setConfig(p => ({ ...p, pais: e.target.value }))}>
                      <option value="" style={{ background: '#111' }}>Seleccionar...</option>
                      {[pais || 'Paraguay', 'Colombia', 'México'].map(o => <option key={o} value={o} style={{ background: '#111' }}>{o}</option>)}
                    </select>
                  </div>
                </div>
