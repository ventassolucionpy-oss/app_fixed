...
function buildSourcingPrompt(data: Record<string, string>): string {
  return `Sos un experto en dropshipping en ${cfg?.nombre || 'Paraguay'} con 5+ años operando con Dropi Paraguay. Conocés el mercado paraguayo a fondo: qué compra la gente, a qué precio, qué genera confianza y qué genera rechazo.

CONTEXTO DROPI PARAGUAY:
- Dropi.co/py es el principal proveedor de dropshipping en ${cfg?.nombre || 'Paraguay'}
...
- La tasa de devolución COD en ${cfg?.nombre || 'Paraguay'} es del 15-25%
...
Respondé SOLO con JSON válido:
{
  "evaluacion": {
    "score": 7.5,
    "potencial": "alto",
    "veredicto": "2-3 líneas directas sobre si este producto tiene potencial real en ${cfg?.nombre || 'Paraguay'} con Dropi — honestidad brutal, sin suavizar",
    "apto_cod": true
  },
  "precio_optimo": {
    "costo_dropi_estimado": "estimación del costo en Dropi en guaraníes si no fue especificado",
    "precio_venta_recomendado": "precio en guaraníes terminado en X7.000 — el precio que maximiza conversión en ${cfg?.nombre || 'Paraguay'}",
    ...
    "justificacion": "por qué ese precio funciona en ${cfg?.nombre || 'Paraguay'} — psicología de precios local"
  },
  "descripcion_checkout": {
    ...
    "garantia": "texto de garantía específico para ${cfg?.nombre || 'Paraguay'} — debe generar confianza ante la desconfianza del mercado",
