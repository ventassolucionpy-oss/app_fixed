...
const CHECKLIST_ITEMS: Omit<CheckItem, 'respuesta'>[] = [
  ...
  { id: 'no_restringido', categoria: 'Logística', pregunta: `¿No tiene restricciones de aduana o importación en ${cfg?.nombre || 'tu país'}?`, descripcion: 'Medicamentos, baterías grandes, ciertos alimentos — pueden frenarse en aduana. Verificar.', peso: 3, critico: true },
  ...
  // Mercado Paraguay
  { id: 'problema_real', categoria: 'Mercado local', pregunta: `¿Resuelve un problema real y frecuente en ${cfg?.nombre || 'tu país'}?`, descripcion: 'Dolor de espalda, manchas, control de peso, cabello. Problemas que el paraguayo quiere resolver HOY.', peso: 3, critico: true },
  { id: 'precio_viable', categoria: 'Mercado local', pregunta: `¿El precio de venta genera margen real con los costos de ${cfg?.nombre || 'tu país'} COD?`, descripcion: 'Por debajo de Gs. 90k la percepción de "muy barato" genera dudas. Por encima de Gs. 250k, la fricción de COD aumenta mucho.', peso: 3, critico: true },
  { id: 'sin_competencia_masiva', categoria: 'Mercado local', pregunta: `¿No está saturado en ${cfg?.nombre || 'tu país'}? (no lo vende la mitad de Dropi ya)`, descripcion: 'Buscá en grupos de Facebook de compradores paraguayos si el producto ya está sobrevendido.', peso: 2, critico: false },
  { id: 'sin_estacion', categoria: 'Mercado local', pregunta: '¿No depende de estacionalidad crítica? (navidad, invierno)', descripcion: 'Productos sin estación son más estables. Estacionales pueden ser picos grandes pero riesgosos.', peso: 1, critico: false },
...
export default function ValidadorProductoPage() {
  ...
  const resultado = useMemo(() => {
    ...
    if (criticosFallados.length >= 2 || score < 40) {
      ...
      veredicto = 'verde'
      mensaje = `Producto validado — listo para lanzar en ${cfg?.nombre || 'tu país'} COD.`
    }
      ...
      <main className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <Link href="/dropi" className="text-white/30 hover:text-white/60 text-sm transition-colors">← Buscar producto</Link>
            <span className="text-white/20 text-sm">›</span>
            <span className="text-white/60 text-sm">Validador</span>
          </div>
          <h1 className="text-2xl font-bold text-white">✅ Validador COD — {cfg.nombre} {cfg.bandera}</h1>
        ...
        <div className="card p-4 border border-amber-500/20 bg-amber-500/5">
          ...
          <p className="text-xs text-white/40">En COD, si no podés responder "sí" a los puntos críticos con confianza, el producto va a costar más en devoluciones y fletes que lo que va a generar. Mejor encontrar un producto mejor.</p>
        </div>
