...
ALTER TABLE profiles
  ADD COLUMN IF NOT EXISTS pais TEXT DEFAULT NULL CHECK (pais IN ('PY','CO','MX')),
  ADD COLUMN IF NOT EXISTS moneda TEXT DEFAULT 'PYG',
  ADD COLUMN IF NOT EXISTS nivel TEXT DEFAULT 'nuevo' CHECK (nivel IN ('nuevo','algo','experto')),
...
-- ─────────────────────────────────────────────
-- 8. TABLA PIPELINE CREATIVOS (para sync multi-dispositivo futuro)
-- ─────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS pipeline_creativos (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  ...
  estado TEXT DEFAULT 'pendiente_guion' CHECK (estado IN (
    'pendiente_guion','guion_listo','en_produccion','entregado',
    'en_revision','aprobado','subido_meta','activo','pausado'
  )),
  fecha_limite DATE,
  fecha_entrega DATE,
  notas TEXT DEFAULT '',
  resultado_roas DECIMAL(5,2),
  resultado_ctr DECIMAL(5,2),
  pais TEXT DEFAULT 'PY' CHECK (pais IN ('PY','CO','MX')),
  created_at TIMESTAMPTZ DEFAULT NOW()
);

ALTER TABLE pipeline_creativos ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "pipeline_own" ON pipeline_creativos;
CREATE POLICY "pipeline_own" ON pipeline_creativos FOR ALL USING (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_pipeline_semana ON pipeline_creativos(user_id, semana);

SELECT 'Schema v6 actualizado con pipeline_creativos' as status;
